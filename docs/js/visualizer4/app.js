...
//
//   p.preload = () => {
//     sound = p.loadSound(`${filePath()}/sound/time_leap_inst_free_ver1.mp3`);
//   };
//
//   p.setup = () => {
...
//       x = p.map(i, 0, spectrumLength, 0, startX + p.map(amplitudeLevel, 0, 1, 0, 500));
//
//       /*
//        * levelの値から直径の値を求める
...
//        */
//       const diameter = p.map((val - average) * tuningMultiply, 0, 255, 0, 50) * tuningMultiply;
//
//       velocity[i] += p.map((val - average) * tuningMultiply, 0, 255, 0, 0.002);
//
//       if (velocity[i] > 0.5) {
//         velocity[i] = 0;
//       }
